...
public class ModeloEntrenador {
    ...
    public boolean registrar(int id, String nombre) throws SQLException {
        ...
        CallableStatement cs = con.prepareCall("{call sp_registrar_entrenador(?, ?)}");
        cs.setInt(1, id);
    ...
    public boolean inhabilitar(int id) throws SQLException {
        Connection con = conexionBDD.conectar();
        CallableStatement cs = con.prepareCall("{call sp_inhabilitar_entrenador(?)}");
        cs.setInt(1, id);
        return cs.executeUpdate() > 0;
    }
    ...
    public ResultSet consultarTodos() throws SQLException {
        Connection con = conexionBDD.conectar();
        CallableStatement cs = con.prepareCall("{call sp_consultar_entrenadores()}");
